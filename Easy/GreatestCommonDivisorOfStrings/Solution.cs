...
{
    public string GcdOfStringsFirstTry(string str1, string str2)
    {
        return GcdOfStringsSecondTry(str1, str2);
    }
    ...
    public string GcdOfStringsSecondTry(string str1, string str2)
    {
        /// 执行用时: 92 ms, 在所有 C# 提交中击败了 84.62% 的用户
        /// 内存消耗: 23.9 MB, 在所有 C# 提交中击败了 20.00% 的用户
    ...
    public string GcdOfStringsThirdTry(string str1, string str2)
    {
        /// 执行用时: 108 ms, 在所有 C# 提交中击败了 30.77% 的用户
        /// 内存消耗: 23.7 MB, 在所有 C# 提交中击败了 20.00% 的用户
        ...
        for (int divisorLength = shorterStr.Length; divisorLength > 0; divisorLength--)
        {
            if (longerStr.Length % divisorLength == 0 && shorterStr.Length % divisorLength == 0)
            {
                string commonDivisor = shorterStr.Substring(0, divisorLength);
                if (longerStr.IndexOf(commonDivisor) != -1)
                {
                    int longerStrIndex = 0;
                    bool found = true;
